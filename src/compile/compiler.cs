...
public class ModuleCompiler : AST_Visitor
{
  ...
  public override void DoVisit(AST_LambdaDecl ast)
  {
    ...
    foreach(var up in lambda.upvals)
      Emit(Opcodes.SetUpval, new int[] {(int)up.upval_idx, (int)up.local_idx, (int)up.mode}, up.line_num);
  }
  ...
  public override void DoVisit(AST_BinaryOpExp ast)
  {
    switch(ast.type)
    {
      ...
      case EnumBinaryOp.EQ:
        ...
        break;
      case EnumBinaryOp.EQ_SCLR:
        VisitChildren(ast);
        Emit(Opcodes.EqualScalar, null, ast.line_num);
        break;
      case EnumBinaryOp.EQ_STR:
        VisitChildren(ast);
        Emit(Opcodes.EqualString, null, ast.line_num);
        break;
