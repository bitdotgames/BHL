...
public class TestInterface : BHL_TestBase
{
  ...
  public void TestUserInterfaceDeepInheritance()
  {
    ...
    var vm = MakeVM(bhl);
    Assert.Equal(4 + 3 + 40 + 30, Execute(vm, "test").Stack.Pop().num);
    CommonChecks(vm);
  }
  ...
  {
    {
      ...
      var expected =
          new ModuleCompiler()
            .UseCode()
            .EmitChain(Opcodes.EnterFrame, new int[] { 1 + 1 /*this*/, 1 })
            ...
            .EmitChain(Opcodes.CallMethodIface, new int[] { 0, TypeIdx(c, c.ns.T("IFoo")), 1 })
            .EmitChain(Opcodes.Return)
        ;
