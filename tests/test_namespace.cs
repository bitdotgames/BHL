...
------------------^"
  ...
  public void TestStartFuncByPath()
  {
    ...
    ";
    var vm = MakeVM(bhl);
    Assert.Equal(1, Execute(vm, "foo.test").result_old.PopRelease().num);
    Assert.Equal(0, Execute(vm, "bar.test").result_old.PopRelease().num);
    CommonChecks(vm);
  }
  ...
  {
    {
      ...
      ";
      var vm = MakeVM(bhl);
      Assert.Equal(1, Execute(vm, "bar.test").result_old.PopRelease().num);
      CommonChecks(vm);
    }

    {
      ...
      ";
      var vm = MakeVM(bhl);
      Assert.Equal(11, Execute(vm, "bar.test").result_old.PopRelease().num);
      CommonChecks(vm);
    }
  }
  ...
  public async Task TestMixNativeAndUserland()
  {
    ...
    {
      {
        var fn = new FuncSymbolNative(new Origin(), "wow", ts.T("int"),
          ...
          {
            stack.Push(ValOld.NewInt(frm.vm, 1));
            return null;
          }
        );
    ...
    Assert.Equal(1, Execute(vm, "bar.wow").result_old.PopRelease().num);
    CommonChecks(vm);
  }
    ...
    var ts_fn = new Action<Types>((ts) =>
    {
      ...
      }
      {
        var fn = new FuncSymbolNative(new Origin(), "wow", ts.T("int"),
          delegate(VM.FrameOld frm, ValOldStack stack, FuncArgsInfo args_info, ref BHS status)
          {
            stack.Push(ValOld.NewInt(frm.vm, 10));
            return null;
          }
        );
  ...
  public void TestPreferLocalVersion()
  {
    ...
    ";
    var vm = MakeVM(bhl);
    Assert.Equal(10, Execute(vm, "foo.test").result_old.PopRelease().num);
    CommonChecks(vm);
  }
...
";
  ...
  public void TestUserClasses()
  {
    ...
    var vm = MakeVM(bhl);
    Assert.Equal(114.5, Execute(vm, "test").result_old.PopRelease().num);
    CommonChecks(vm);
  }
...
-----------------^"
  ...
  public async Task TestImportSeveralNestedNamespacesWithSameLastName()
  {
    ...
    vm.LoadModule("bhl2");
    Assert.Equal(120, Execute(vm, "test").result_old.PopRelease().num);
    CommonChecks(vm);
  }
