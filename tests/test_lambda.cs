...
----------------------------------------------^"
  ...
  public void TestLambdaCallInFalseCondition()
  {
    ...
    var fb = vm.Start("test");
    Assert.False(vm.Tick());
    Assert.Equal(321, fb.result_old.PopRelease().num);
    CommonChecks(vm);
  }
  ...
  public void TestLambdaCallAsVarSeveralTimes()
  {
    ...
    var fb = vm.Start("test");
    Assert.False(vm.Tick());
    Assert.Equal(246, fb.result_old.PopRelease().num);
    CommonChecks(vm);
  }
  ...
  public void TestLambdaCallAsVarWithArgs()
  {
    ...
    var fb = vm.Start("test");
    Assert.False(vm.Tick());
    Assert.Equal(41, fb.result_old.PopRelease().num);
    CommonChecks(vm);
  }
...
----------------------------------^"
  ...
  public void TestLambdaCapturesSeveralVars()
  {
    ...
    var expected =
        new ModuleCompiler()
          ...
          .EmitThen(Opcodes.Return)
          //test
          .EmitThen(Opcodes.InitFrame, new int[] { 2, 0 })
          ...
          .EmitThen(Opcodes.InitFrame, new int[] { 3, 1 })
    ...
    var fb = vm.Start("test");
    Assert.False(vm.Tick());
    Assert.Equal(35, fb.result_old.PopRelease().num);
    CommonChecks(vm);
  }
  ...
  public void TestReturnMultipleLambda()
  {
    ...
    Assert.Equal(30, fb.result_old.PopRelease().num);
    AssertEqual(fb.result_old.PopRelease().str, "foo");
    CommonChecks(vm);
  }
